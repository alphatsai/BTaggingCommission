...
def main():
  ...
  parser.add_option("-a", "--analyzer_module", dest="analyzer_module",
                    help="Name of the analyzer module",
                    default='',
                    metavar="ANALYZER_MODULE")

  parser.add_option("-e", "--evt_histo", dest="evt_histo",
                    help="Event histogram",
                    default='TotalGenEvts',
                    metavar="EVT_HISTO")
  ...
  for group in groups:
    ...
    for dataset in group_datasets[group]:
      ...
      htemp = root_file.Get(os.path.join(options.analyzer_module,options.evt_histo))
      ...
      if group_xs[group] > 0.:
        ...
        print dataset + ' -- Events: %.0f (all); relative xs: %.8E; scale: %.8E'%(nEventsAll,(dataset_xs[dataset]/group_xs[group]),scale)
      else:
        print dataset + ' -- Events: %.0f (all); scale: %.8E'%(nEventsAll,scale)
      ...
      for h in range(0, nHistos):
        ...
        if htemp.InheritsFrom('TH1') or htemp.InheritsFrom('TH2'):
          ...
          if histoName not in final_histos.keys():
              ...
              final_histos[histoName].SetName(histoName + '_' + group)
